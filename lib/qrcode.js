/* qrcode.js — generator QR code murni JS, tanpa dependensi/CDN eksternal.
 * Basis: algoritma qrcode-generator (Kazuhiko Arase, MIT License) —
 * dipakai lokal supaya barcode tetap muncul walau CDN diblokir/offline.
 * API: qrcodeDraw(canvas, text, { size, margin, dark, light, ecLevel }) */
(function (global) {
  "use strict";
  ...
  var QRRSBlock = (function () {
    ...
    qrRSBlock.getRSBlocks = function (typeNumber, errorCorrectLevel) {
      ...
      for (var i=0;i<length;i++) { var count=rsBlock[i*3+0], totalCount=rsBlock[i*3+1], dataCount=rsBlock[i*3+2];
  ...
  function QR8BitByte(data) {
    ...
    for (var i=0;i<utf8.length;i++) this.parsedData.push(utf8.charCodeAt(i)&0xff);
  }
  ...
  QRCodeModel.prototype = {
    ...
    setupPositionProbePattern: function (row,col) {
      for (var r=-1;r<=7;r++) { if (row+r<=-1||this.moduleCount<=row+r) continue; for (var c=-1;c<=7;c++) { if (col+c<=-1||this.moduleCount<=col+c) continue;
  ...
  function getUtf8Length(s) { return unescape(encodeURIComponent(s)).length; }
  function pickTypeNumber(text, ecLevel) {
    var len = getUtf8Length(text);
    for (var t=1; t<=40; t++) {
      try {
        var model = new QRCodeModel(t, ecLevel);
        model.addData(text);
        model.make();
        return model;
      } catch (e) { /* terlalu kecil, coba versi berikutnya */ }
    }
    throw new Error("Data too long for QR code");
  }

  /** Gambar QR code ke <canvas>.
   *  opts: { size(px), margin(modul), dark, light, ecLevel: 'L'|'M'|'Q'|'H' } */
  function qrcodeDraw(canvas, text, opts) {
    opts = opts || {};
    var size = opts.size || 200;
    var margin = opts.margin != null ? opts.margin : 2;
    var dark = opts.dark || "#000000";
    var light = opts.light || "#ffffff";
    var ec = QRErrorCorrectLevel[(opts.ecLevel || "M").toUpperCase()];
    var model = pickTypeNumber(text, ec);
    ...
    for (var row=0; row<count; row++) {
      for (var col=0; col<count; col++) {
        if (model.isDark(row,col)) {
          var x = Math.round((col+margin)*cell), y = Math.round((row+margin)*cell);
          var w = Math.ceil((col+margin+1)*cell)-x, h = Math.ceil((row+margin+1)*cell)-y;
